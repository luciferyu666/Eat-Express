...
const OrderDetail = ({ order, updateOrderStatus }) => {
  ...
  return (
    <div>
      ...
      <p><strong>聯繫電話：</strong>{order.contact}</p>
      ...
      <p><strong>送餐時間：</strong>{order.deliveryTime}</p>
      <div className="mt-2">
        <label className="block text-gray-700">訂單狀態：</label>
        <select value={currentStatus} onChange={handleStatusChange} className="border rounded px-2 py-1">
