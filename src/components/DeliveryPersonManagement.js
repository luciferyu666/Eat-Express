...
const DeliveryPersonManagement = () => {
  ...
  const handleSave = () => {
    axios.put(`/api/delivery-persons/${editingPerson.id}`, editingPerson)
      .then(() => {
        setEditingPerson(null);
        alert('外送員資料已更新');
      })
      .catch(error => console.error('Error saving delivery person data:', error));
  };

  const handleDisable = (personId) => {
    if (window.confirm('確定禁用此外送員嗎？')) {
      axios.put(`/api/delivery-persons/${personId}/disable`)
        .then(() => {
          setDeliveryPersons(deliveryPersons.filter(person => person.id !== personId));
          alert('外送員已禁用');
        })
        .catch(error => console.error('Error disabling delivery person:', error));
    }
  };
