 ...
 * 並根據 role 屬性區分不同用戶類型。此腳本專門針對 middleware 目錄下的中間件文件。
 */
...
module.exports = function (fileInfo, api, options) {
    ...
    // 5. 確保中間件使用 User 模型進行角色驗證
    // 查找所有 authorizeRoles 中間件的定義，確保使用 User 模型
    root.find(j.VariableDeclarator, {
      id: { name: 'authorizeRoles' }
    }).forEach(path => {
      // 確保內部使用 User.findById
      j(path).find(j.CallExpression, {
        ...
        hasModifications = true;
        console.log(`Ensured User.findById usage in ${fileInfo.path}`);
      });
    });
  
    // 6. 添加詳細的 console.log 語句
